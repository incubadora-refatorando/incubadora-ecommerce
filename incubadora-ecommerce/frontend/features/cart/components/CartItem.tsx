...
export function CartItem({ item }: CartItemProps) {
	...
	const handleUpdateQuantity = async (newQuantity: number) => {
		...
		if (newQuantity > item.product.stock) {
			toast.error(
				`Apenas ${item.product.stock} unidades disponíveis em estoque`,
			);
			return;
		}
	...
	return (
		<div className="flex gap-4 p-4 bg-white rounded-lg border border-border">
			...
			<Link
				...
				className="relative w-24 h-24 shrink-0 bg-gray-light rounded-lg overflow-hidden">
			...
			<div className="flex-1 flex flex-col justify-between">
				...
				<div className="flex items-center gap-3 mt-2">
					<div className="flex items-center border border-border rounded-lg">
						...
						<span className="px-4 py-1 text-sm font-medium border-x border-border min-w-12 text-center">
							{item.quantity}
						</span>
						<Button
							variant="ghost"
